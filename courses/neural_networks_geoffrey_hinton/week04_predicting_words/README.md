...
Let's say that a network with $$n$$ linear output units has some weights $$w$$. $$w$$ is a matrix with $$n$$ columns, and $$w_i$$ indexes a particular column in this matrix and represents the weights from the inputs to the \_i\_th output unit.
